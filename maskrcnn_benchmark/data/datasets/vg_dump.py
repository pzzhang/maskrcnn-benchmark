...
class VGDataset(torch.utils.data.Dataset):
    ...
    def _dump_tsv(self):
        ...
        print("create file %s"%(cache_file))
        print("create file %s"%(idwh_file))
        idwh_mat = np.zeros((len(self.ids),3), dtype=int)
        with open(cache_file,'w') as fout:
            for index in range(len(self.ids)):
                ...
                idwh_mat[index,0] = img_id
                idwh_mat[index,1] = width
                idwh_mat[index,2] = height
        np.save(idwh_file, idwh_mat)
        print("finish writing file %s"%(cache_file))
        print("finish writing file %s"%(idwh_file))
